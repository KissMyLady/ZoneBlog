...
from ..utils import ZONE_BLOG_TOOLS
...
def get_toollist_by_key(key=None):
    """返回工具列表"""
    tools = []
    if not key or key not in ZONE_BLOG_TOOLS:
        for _k in ZONE_BLOG_TOOLS:
            _tag = ZONE_BLOG_TOOLS[_k]['tag']
            _tools = ZONE_BLOG_TOOLS[_k]['tools']
            for each in _tools:
                item = {'tag': _tag,
                        'name': each['name'],
        ...
        _tag = ZONE_BLOG_TOOLS[key]['tag']
        _tools = ZONE_BLOG_TOOLS[key]['tools']
        for each in _tools:
            item = {'tag': _tag,
                    'name': each['name'],
                    'url': reverse(each['url']),
